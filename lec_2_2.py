...
# list_1 = [exp for item in iterable], где exp - значение, iterable - количество итераций
# Рапример:
# Задача.
# Создать список, состоящий из чётных чисел в диапазоне от 1 до 100.
# Решение:

# # 1. Сщздаём список чисел от 1 до 100
# list_1 = []
...
# Допустим, создадим пары каждому из чисел (кортежи):
list_1 = [(i, i) for i in range(1, 101) if i % 2 == 0]
print(list_1)

# Также можно умножать, делить, прибовлять, вычитать.
# Например, умножим значение на 2:
